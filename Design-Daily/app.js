...
const collage=a=>{
 ...
 return `<svg class="collage" viewBox="0 0 800 500" role="img" aria-label="极简几何拼贴缺省封面"><rect width="800" height="500" fill="#eeeee5"/>${shapes[n]}<path d="M70 440h660" stroke="#9ba08f"/><text x="70" y="470" fill="#656d59" font-size="12" font-weight="400" letter-spacing="2">设计博物馆 · 灵感拼贴</text></svg>`;
};
...
async function start(){try{const r=await fetch('./data.json',{cache:'no-cache'});if(!r.ok)throw Error('HTTP '+r.status);data=await r.json();if(!data.issues?.length)throw Error('No editions');const params=new URLSearchParams(location.search);selectedDate=params.get('date')||data.issues.map(i=>i.date).sort().at(-1);if(!/^\d{4}-\d{2}-\d{2}$/.test(selectedDate))selectedDate=data.issues.map(i=>i.date).sort().at(-1);category=CATEGORIES.includes(params.get('category'))?params.get('category'):'全部';sort=params.get('sort')==='new'?'new':'hot';$('updatedAt').textContent='更新于 '+new Date(data.updatedAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false});$('scheduleNote').textContent=data.schedule?.active?'每天 09:00（北京时间）更新':'每日简报计划：北京时间 09:00，推送尚未启用。';render();if(params.get('article'))openArticle(params.get('article'),false)}catch(e){$('news').innerHTML='<div class="empty-state"><h2>日报暂时没有展开。</h2><p>资讯读取失败，请稍后重试。</p><button id="retry">重新加载</button></div>';$('retry').onclick=start;$('articleCount').textContent='连接暂时中断'}}
