...
function render(){
 ...
 $('categories').innerHTML=CATEGORIES.map(c=>`<button data-category="${c}" class="${category===c?'active':''}" aria-pressed="${category===c}">${c}<small>${c==='全部'?issue.length:issue.filter(a=>a.category===c).length}</small></button>`).join('');
...
$('aboutButton').onclick=()=>showInfo('设计，每天发生。','<p>设计日报 是一份跨领域的设计资讯日报。从工业产品到交互界面，从动态影像到家具与出行，每天整理设计的新动向。</p><p>首页浏览简介，详情页阅读中文整理与设计解读，也可前往原文深入了解。日报按采集日期归档，各条资讯标注原始发布日期。</p><p>人工智能设计栏目收录人工智能工具、生成式创作与相关设计实践。动效关注交互与界面运动，动态设计关注影像、动画与视觉叙事。</p>');
...
function showListing(){activeArticle=null;$('listingPage').hidden=false;$('articlePage').hidden=true;document.title='设计日报 · 每天发现好设计'}
function openArticle(id,push=true){
 ...
 document.title=title+' · 设计日报';
 $('articlePage').innerHTML=`<nav class="detail-breadcrumb" aria-label="当前位置"><a href="${listURL()}" data-back>← 返回日报</a><span> / </span><span>${escapeHTML(a.category)}</span></nav><header class="detail-header"><div class="detail-kicker">${escapeHTML(a.category)}<span>第 ${String(data.articles.indexOf(a)+1).padStart(3,'0')} 条资讯</span></div><h1>${escapeHTML(title)}</h1><div class="detail-meta"><span>来源：${escapeHTML(src)}</span><time datetime="${escapeHTML(a.publishedAt)}">${fmtDate(a.date)} 发布</time><span>收录于 ${fmtDate(a.firstSeen)}</span></div></header><div class="detail-layout"><article class="detail-content">${a.image?`<figure><img src="${safeURL(a.image)}" alt="${escapeHTML(title)}"><figcaption>图片来源：${escapeHTML(src)} · 图片版权归原作者所有</figcaption></figure>`:''}<section class="detail-summary"><span class="eyebrow">中文整理 · 设计日报</span><p class="detail-lead">${escapeHTML(a.summaryZh)}</p>${(a.reading?.sections||[]).map(section=>`<section class="reading-section"><h2>${escapeHTML(section.heading)}</h2>${section.paragraphs.map(p=>`<p>${escapeHTML(p)}</p>`).join('')}</section>`).join('')}<p class="editorial-note">${a.reading?.basis==='article'?'根据原文独立整理。':'资讯事实依据来源公开摘要整理。'}设计解读为本站编辑观点。完整原文、项目图片及影像请前往来源查看。</p><a class="reading-original" href="${safeURL(a.url)}" target="_blank" rel="noopener noreferrer">前往${escapeHTML(src)}阅读原文 ↗</a></section><a class="detail-back" href="${listURL()}" data-back>← 返回本期，继续发现</a></article><aside class="detail-aside"><div class="original-card"><span class="eyebrow">继续深入阅读</span><h2>设计的细节，<br>值得多看一眼。</h2><p>前往${escapeHTML(src)}查看完整内容。</p><a class="primary-link" href="${safeURL(a.url)}" target="_blank" rel="noopener noreferrer">阅读原文 <span>↗</span></a><small>原文网站可能使用外文。</small></div><div class="detail-index"><span>本期关注指数</span><strong>${a.score}<small> / 100</small></strong><p>依时效与内容完整度估算，<br>不是全网浏览量或点赞量。</p></div></aside></div>`;
 const img=$('articlePage').querySelector('figure img');if(img)img.addEventListener('error',()=>{img.closest('figure').remove()},{once:true});
}
document.addEventListener('click',e=>{const a=e.target.closest('a[data-article],a[data-back]');if(!a||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;e.preventDefault();if(a.hasAttribute('data-back')){history.pushState(null,'',listURL());showListing();render();window.scrollTo(0,0)}else openArticle(a.dataset.article)});
